...
let intersectionsList = document.querySelector('.intersections');

function searchIntersections() {
  let searchField = document.querySelector('.search-field');
  let value = searchField.value;

  if (value == '') intersectionsList.classList.remove('hidden');
...
function downloadData(page = 1, autocompleteValue) {
  ...
  if (autocompleteValue == '') intersectionsList.classList.remove('hidden');

  url.searchParams.append('page', page);
  url.searchParams.append('per-page', perPage);
  // Добавляем параметр q с значением автокомплита (если оно есть)
  url.searchParams.append('q', autocompleteValue ? autocompleteValue : value);
